...
const EventsPage: React.FC<Props> = ({ user, onLogout }) => {
  ...
  return (
    <AppLayout user={user} onLogout={onLogout} sidebarContext={sidebarContext}>
      <Header>
        <Title>Events</Title>
        <Subtitle>{user.role === 'admin' ? 'Manage your golf events, players, and finances' : 'View golf events and finances'}</Subtitle>
      </Header>

      {error && <ErrorContainer>{error}</ErrorContainer>}

      {!loading && events.length > 0 && <ExportActions events={events} />}

      {loading ? (
        <LoadingContainer>Loading events...</LoadingContainer>
      ) : activeEvent ? (
        <EventDetail event={activeEvent} participants={participants} isAdmin={user.role === 'admin'} />
      ) : (
        <LoadingContainer>
